...
export class LitButton extends focusable(stylable(definable(LitElement), buttonCCSVarsMap, PREFIX)){
    ...
    static styles = [
        ...
        :host(:not([type=switch])[pressed]) .wrapper{
            background: radial-gradient(circle at var(--click-x) var(--click-y), ${LitButton.cssVar('ripple')} 5px , ${LitButton.cssVar('background-hover')} var(--radiant));
        }
        ...
        :host([primary][pressed]) .wrapper{
            background: radial-gradient(circle at var(--click-x) var(--click-y), ${LitButton.cssVar('primary-ripple')} 5px , ${LitButton.cssVar('primary-background-hover')} var(--radiant));
        }
        ...
        :host([success][pressed]) .wrapper{
            background: radial-gradient(circle at var(--click-x) var(--click-y), ${LitButton.cssVar('success-ripple')} 5px , ${LitButton.cssVar('success-background-hover')} var(--radiant));
        }
        ...
        :host([danger][pressed]) .wrapper{
            background: radial-gradient(circle at var(--click-x) var(--click-y), ${LitButton.cssVar('danger-ripple')} 5px , ${LitButton.cssVar('danger-background-hover')} var(--radiant));
        }
        `
    ]
    ...
    public click(): void{  
        ...
        if(this.type === 'close'){
            this.dispatchEvent(new CustomEvent<ButtonEvents['buttonClose']>('buttonClose', {
                bubbles: true,
                composed: true
            }))
        }
    ...
    public toggleSwitch(): void{
        ...
        this.dispatchEvent(new CustomEvent<ButtonEvents['switchChanged']>('switchChanged', {
            detail: this.pressed,
            bubbles: true,
        }))
    }
    ...
    public submit(): void{
        ...
        this.dispatchEvent(new CustomEvent<ButtonEvents['submitForm']>('submitForm', {
            bubbles: true,
            composed: true,
        }))
        this._notifyOnClick()
