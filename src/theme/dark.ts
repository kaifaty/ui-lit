...
export const darkStyles = css`
        :host([theme = "dark"]){
            ...
            ${text.colorDanger}: var(--negative-0);
            ${text.colorSuccess}: var(--positive-0);
            ${text.colorAccented}: var(--positive-0);

            ${icon.color}: var(--primary--650);
            ${icon.positiveColor}: var(--positive-0);
            ${icon.negativeColor}: var(--negative-0);

            ${circle.color}: var(--primary--100);

            ${link.color}: var(--primary--100);
            ${link.colorHover}: var(--primary--300);


            ${divider.color}: var(--primary-800);

            ${scrollbarCSSVarNames.thumb}: var(--primary-1000);
            ${scrollbarCSSVarNames.track}: var(--primary-1500);
            ...
            ${button.color}: var(--primary--600);
            ${button.background}: initial;
            ${button.backgroundHover}: var(--primary-1200);
            ${button.ripple}: var(--primary-900);
            ${button.backgroundFocus}: var(--primary-300);
            ${button.border}: 1px solid var(--primary-900);
            ${button.outlineFocus}: 1px solid var(--primary-700);


            ${button.switchColor}: var(--primary--600);
            ${button.switchOnColor}: var(--primary--600);
            ${button.switchOnBackground}: var(--primary-400);

            ${button.switchOutlineFocus}: 1px solid var(--primary-700);
            ${button.switchOnOutlineFocus}: 1px solid var(--primary-700);
            ...
            ${range.filled}: var(--primary-300);
            ${range.filledHover}: var(--primary-0);

            ${range.trackBackground}: var(--primary-1100);
            ${range.trackBackgroundHover}: var(--primary-900);
            ${range.trackOutline}: 1px solid var(--primary-1050);

            ${range.thumbBackground}: var(--primary-1200);
            ${range.thumbBackgroundHover}: var(--primary-800);
            ${range.thumbBackgroundPressed}: var(${range.filledHover});
            ${range.thumbOutline}: 2px solid var(--primary--600);
            ${range.thumbShadow}: 1px 1px 8px var(--primary-1450);
            ${range.pointBackground}: var(--primary-1300);
            ...
            ${menuCSSVarsNames.color}: var(--primary--650);
            ${menuCSSVarsNames.background}: var(--primary-1200);
            ${menuCSSVarsNames.backgroundHover}: var(--primary-1000);
            ${menuCSSVarsNames.ripple}: var(--primary-800);


            ${select.color}: var(--primary--650);
            ${select.background}: initial;
            ${select.backgroundHover}: var(--primary-1200);
            ${select.ripple}: var(--primary-900);
            ${select.outlineFocus}: 1px solid var(--primary-700);
            
            ${select.border}: 1px solid var(--primary-900);
            ${select.circleColor}: var(--primary-500);
            ${select.optionColor}:  var(--primary--700);
            ${select.optionBackground}: var(--primary-1200);
            ${select.optionBackgroundHover}: var(--primary-1100);
            ${select.optionRipple}: var(--primary-800);
            ${select.listboxBorder}: 1px solid var(--primary-900);
            ${select.listboxShadow}: var(--primary-1100);
            ...
            ${table.rowSelected}: var(--primary-1150);
            ...
            ${checkbox.border}: 2px solid var(--primary-800);
            ${checkbox.checkmarkColor}: var(--primary--650);
            ${checkbox.background}: var(--primary-1300);
            ${checkbox.shadowHover}: 0 0 5px var(--primary-800);
            ${checkbox.switcherControlBackground}: var(--primary--700);
            ${checkbox.switcherControlShadow}: 1px 1px 5px var(--primary-1300);
            ...
            ${dialog.backgroundColor}: var(--primary-1300);
            ${dialog.color}: var(--primary--650);
